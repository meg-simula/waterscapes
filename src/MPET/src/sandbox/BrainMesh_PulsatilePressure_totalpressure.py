...
class FirstTest(MPET):
    ...
    def initial_conditions(self, t):
        ...
        #initial conditions are zero, so also for total pressure 
        return uex, [pex[0], pex[0]]
    ...
    def boundary_conditions(self, t):
        ...
        p0 = [((2), self.CSF_pressure(t), self.allboundary)]
...
def single_run(paramsfile, mesh):
    ...
    foldername = "Mesh_%s"%str(mesh)[6:] + "_PulsatilePressure_Acceleration_%s"%problem.params.Acceleration + "_nullspace_%s" %problem.nullspace() +\
                 "_E_" + "%05.03e" %problem.params.E + "_nu_" + "%04.05e" %problem.params.nu +"_K_" + "%04.05e" %problem.params.Ks[0] +\
                 "_Q" + "%04.03e" %problem.params.Q + "_totalpressure"
