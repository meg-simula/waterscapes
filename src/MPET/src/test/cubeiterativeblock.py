...
class FirstTest(MPET):
    ...
    def boundary_conditions(self, t):
        ...
        # u0 = [(Expression(("1.0","0.0", "0.0"), domain=self.mesh, degree=5, t=t, L=L), self.allboundary)]
        p0 = [((1), Expression("1.0 + a*sin(pi/30.0 * t)", domain=self.mesh, degree=5, t=t, L=L, a=1.0), self.allboundary),\
              ((2), Expression("1.0 + a*sin(pi/30.0 * t)", domain=self.mesh, degree=5, t=t, L=L, a=1.0), self.allboundary) ]
    ...
    def nullspace(self):
        ...
        # null = [Constant((1, 0, 0)), Constant((0, 1, 0)), Constant((0, 0, 1)),\
        #              Expression(('0', 'x[2]', '-x[1]')), Expression(('-x[2]', '0', 'x[0]')), Expression(('x[1]', '-x[0]', '0'))]
