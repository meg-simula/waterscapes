...
def single_run(N=64, M=64):
    ...
    # Create solver
    solver = SimpleSolver(problem, {"dt": dt, "T": T, "theta":1.0, "direct_solver": True, "u_degree": 2, "p_degree":1})

    foldername = "RodrigoTest_TotalPressureSolver_P2P1"
    ...
    for (U, t) in solutions:
        ...
        plot(p[1], key="p", mesh=problem.mesh)

    plot(MyExpression(), mesh=problem.mesh)

    File("results/" + foldername + "/Ks.pvd")
