...
class FirstTest(MPET):
    ...
    def initial_conditions(self, t):
        ...
        u0 = Expression(('x[0]*x[1]','sin(pi*x[0])'))
        p0 = [Expression('x[0]') for i in range(AA)]
    ...
    def boundary_conditions(self, t):
        ...
        u0 = [(Expression(('x[0]*x[1]','sin(pi*x[0])')), self.allboundary)]
        p0 = [((1), Expression('x[0]'), self.allboundary),\
              ((2), Expression('x[0]'), self.allboundary)]
...
def single_run():
    ...
    #Solve only one time step to test preconditioner
    ...
    for N in [8, 16, 32, 64, 128]:
        ...
        # Create solver
        # Set fieldsplit = True to use fieldsplit options
        # Set fieldsplit = False to use 
        ...
        # Uncomment this if you want to solve using krylov solver
        ...
        # Uncomment this to solve using cbcblock MinRes2
