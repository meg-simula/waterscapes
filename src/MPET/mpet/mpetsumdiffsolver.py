...
class MPETSumDiffSolver(object):
    ...
    def create_variational_forms(self):
        ...
        F = inner(2*mu*sym(grad(u)), sym(grad(v)))*dx() \
          ...
          + sum([-A*theta*dt*S[0][i]*pm[i+1]*w[i+1] for i in As1])*dx()
        ...
        # Define preconditioner form:
            ...
            pp = (c[0]/A + alpha[0]*alpha[0]/lmbda)*p[1]*w[1]*dx() + dt*theta*K[0]/A*inner(grad(p[1]), grad(w[1]))*dx() \
               + sum([-dt*theta*K[i]*inner(grad(p[i+1]), grad(w[i+1])) for i in As1])*dx() \
               + sum([-A*theta*dt*S[0][i]*p[i+1]*w[i+1] for i in As1])*dx()
            ppt = p[0]*w[0]*dx()
            P = pu + pp + ppt
