...
class FirstTest(MPET):
    ...
    def exact_solutions(self, t):
        ...
        u = Expression(("x[0]*sin(2*pi*t)/(1000.0 * rS)","x[1]*sin(2*pi*t)/(1000.0 * rS)"), rS =self.rS, t=t, domain=self.mesh, degree=3)
        p = [Constant(0.0), ]
    ...
    def boundary_conditions(self, t):
        ...
        p0 = [((1), Constant(0.0), self.ventricles),]
        u0 = [(uex, self.skull),]
...
def single_run(paramsfile, mesh):
    ...
    foldername = "2D_1Net_PulsatileDisplacementOnSkull_ConstantPressureOnVentricles_Acceleration_%s"%problem.params.Acceleration + "_nullspace_%s" %problem.nullspace() +\
