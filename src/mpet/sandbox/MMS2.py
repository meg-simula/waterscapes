...
class FirstTest(MPET):
    ...
    def boundary_conditions(self, t):
        ...
        u0 = [(Expression(("sin(2*pi*x[0]/L)*sin(2*pi*x[1]/L)*sin(n*t + 1.0)","sin(2*pi*x[0]/L)*sin(2*pi*x[1]/L)*sin(n*t + 1.0)"),\
                          domain=self.mesh, degree=5, t=t, L=L, n=n), self.uboundary)]
        
        p0 = [((i+1), Expression("(i+1)*sin(2*pi*x[0]/L)*sin(2*pi*x[1]/L)*sin(n*t + 1.0)", domain=self.mesh, degree=5, t=t, L=L, i=i, n=n), self.pboundary) for i in range(AA)]
    ...
    def neumann_conditions(self, t0, t1, theta):
        ...
        p0 = [(Expression("theta * Ks*(2.0*pi*sin(2*pi*x[1]/L)*sin(n*t_dt + 1.0)*cos(2*pi*x[0]/L)/L) + (1.0-theta)*Ks*(2.0*pi*sin(2*pi*x[1]/L)*sin(n*t + 1.0)*cos(2*pi*x[0]/L)/L)", \
                            t=t0, t_dt=t1, theta=theta, nu=nu, E=E, Ks = Ks[0], L=L, n=n), 0, 5),
              (Expression("theta * Ks * (4.0*pi*sin(2*pi*x[1]/L)*sin(n*t_dt + 1.0)*cos(2*pi*x[0]/L)/L) + (1.0-theta)* Ks *(4.0*pi*sin(2*pi*x[1]/L)*sin(n*t + 1.0)*cos(2*pi*x[0]/L)/L)", \
                            t=t0, t_dt=t1, theta=theta, nu=nu, E=E, Ks = Ks[1], L=L, n=n), 1, 5)]
...
def main():
    ...
    kappa1 = Constant(1.0e-6)
    kappa2 = Constant(1.0e-6)
    gamma = Constant(1.0)   
    nu = Constant(0.35)
