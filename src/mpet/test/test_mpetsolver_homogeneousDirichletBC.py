...
def exact_solutions(params):
    ...
    u = [sin(2*pi*x[0])*sin(2*pi*x[1])*sin(omega*t + t),
         sin(2*pi*x[0])*sin(2*pi*x[1])*sin(omega*t + t)]
    p = []
    for i in range(J):
        p += [-(i+1)*sin(2*pi*x[0])*sin(2*pi*x[1])*sin(omega*t + t)]

    # Simplify symbolics 
    d = len(u)
    u = [sympy.simplify(u[i]) for i in range(d)]
    p = [sympy.simplify(p[i]) for i in range(J)]
    ...
    grad_p = [[diff(p[i], x[j]) for j in range(d)] for i in range(J)]
...
def single_run(n=8, M=8, theta=1.0, direct_solver=True):
    ...
    problem.p_bar = [Expression(p_e[i], t=time, degree=4) for i in range(J)]
    ...
    print(solver.params["direct_solver"])
    ...
    p_err_L2 = [errornorm(problem.p_bar[i], p[i], "L2", degree_rise=3) for i in range(J)]
    p_err_H1 = [errornorm(problem.p_bar[i], p[i], "H1", degree_rise=3) for i in range(J)]
